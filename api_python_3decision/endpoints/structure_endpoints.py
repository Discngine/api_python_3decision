...
def _get_viewable_structures_by_id(session, user_id : int, structure_ids : list):
    session.check_token_expiration()
    endpoint_path = '/structure/id/user/:user_id/canview'
    url = session.api_base_internal_url + endpoint_path
    url = url.replace(':user_id', user_id)
    response = session.req.get(url = url, data = {'structure' : structure_ids})
    if response.status_code > 202:
        session.print_error_message(response)
    return response     
